...
export class StreamableHttpTransport extends StatefulTransport {
    initialize(_options) {
        this.setupRoutes();
    ...
    async handleRequest(req, res, method) {
        try {
    ...
    async handlePostRequest(req, res, sessionId) {
        try {
    ...
    async handleGetRequest(req, res, sessionId) {
        if (!sessionId || !this.sessions.has(sessionId)) {
            res.status(400).json({
                jsonrpc: "2.0",
                error: {
    ...
    async handleDeleteRequest(req, res, sessionId) {
        if (!sessionId || !this.sessions.has(sessionId)) {
            res.status(404).json({
                jsonrpc: "2.0",
                error: {
    ...
    async createSession(user = 'default') {
     ...
     */
    async removeStaleSession(sessionId) {
