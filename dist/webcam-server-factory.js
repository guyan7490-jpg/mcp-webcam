...
export let clients = new Map(); // user -> clientId -> response
export let captureCallbacks = new Map(); // user -> clientId -> callback
...
export function getUserClients(user) {
    if (!clients.has(user)) {
        clients.set(user, new Map());
    }
    return clients.get(user);
}
export function getUserCallbacks(user) {
 ...
 */
export const createWebcamServer = async (user = 'default') => {
    ...
    mcpServer.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        const userClients = getUserClients(user);
        if (userClients.size === 0) {
            throw new Error(`No clients connected for user '${user}'. Please visit ${getMcpHost()}${user !== 'default' ? `?user=${user}` : ''} and enable your Webcam.`);
        }
        // Validate URI
    ...
    mcpServer.tool("capture", "Gets the latest picture from the webcam. You can use this " +
    ...
    mcpServer.tool("screenshot", "Gets a screenshot of the current screen or window", {}, {
        openWorldHint: true,
        readOnlyHint: true,
