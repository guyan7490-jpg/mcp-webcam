...
import { createWebcamServer, getUserClients, getUserCallbacks } from "./webcam-server-factory.js";
...
async function processSamplingRequest(imageDataUrl, prompt = "What is the user holding?", sessionId) {
    ...
    try {
        ...
        if (isStreamingMode && sessionId) {
            ...
            server = transport.sessions?.get(sessionId)?.server?.server;
            if (!server) {
        ...
        if (!server.createMessage) {
            throw new Error("Server does not support sampling - no MCP client with sampling capabilities connected");
        }
    ...
    try {
        ...
        if (isStreamingMode && transport) {
            ...
            if (!selectedSessionId || !sessions.find(s => s.id === selectedSessionId)) {
                ...
                const sortedSessions = sessions.sort((a, b) => b.connectedAt.getTime() - a.connectedAt.getTime());
                selectedSessionId = sortedSessions[0]?.id;
            }
            Logger.info(`Using session ${selectedSessionId} for sampling (user: ${user}`);
        }
        const result = await processSamplingRequest(image, prompt, selectedSessionId);
